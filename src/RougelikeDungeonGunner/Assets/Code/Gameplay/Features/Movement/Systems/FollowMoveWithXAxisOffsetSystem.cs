...
{
	public class FollowMoveWithXAxisOffsetSystem : IExecuteSystem
	{
		private readonly GameContext _game;
		private readonly IGroup<GameEntity> _followers;

		public FollowMoveWithXAxisOffsetSystem(GameContext game)
		{
			_game = game;
			...
			foreach (GameEntity follower in _followers)
			{
				...
				if (target != null && target.hasWorldPosition)
					follower.ReplaceWorldPosition(new Vector3(
						...
						target.WorldPosition.y + follower.FollowMovementXAxisOffset));
			}
		}
	}
}